...
from determined import _checkpointing, _preemption, _searcher, _training, tensorboard, workload
...
class WorkloadSequencer(workload.Source):
    ...
    def __init__(
        ...
        self.checkpointing = _checkpointing.Checkpointing(
            session, api_path, static_metadata, tbd_mgr
        )
    ...
    def load_state(self, state: Any) -> None:
        # Discard state from a previous trial.
        ...
        if self.state.total_batches == self.val_from_previous_run:
    ...
    def train(self, num_batches: int, op: _searcher.SearcherOp) -> WorkloadGenerator:
        ...
        if op.unit == _searcher.Unit.BATCHES:
            op.report_progress(self.state.total_batches)
        elif op.unit == _searcher.Unit.RECORDS:
            op.report_progress(self.global_batch_size * self.state.total_batches)
        elif op.unit == _searcher.Unit.EPOCHS:
            op.report_progress(self.state.total_batches / self.as_batches(epochs=op.epochs))
        else:
    ...
    def checkpoint(self, already_exiting: bool) -> WorkloadGenerator:
        ...
        if isinstance(response, workload.InvalidHP):
            # XXX: do we need to protect against this being called twice?
            self.training.report_early_exit(_training.EarlyExitReason.INVALID_HP)
    ...
    def batches_until_val(self) -> int:
        return self.state.last_val + self.min_val_period_batches - self.state.total_batches
    ...
    def __iter__(self) -> workload.Stream:
        self.preemption = _preemption.Preemption(self.session, self._allocation_id, self._dist)
        self.preemption.start()
        try:
            searcher = _searcher.AdvancedSearcher(
            ...
            if (
                ...
                and self.state.total_batches == 0
            ):

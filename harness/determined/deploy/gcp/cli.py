...
def handle_dump_master_config_template(args: argparse.Namespace) -> None:
    fn = pkg_resources.resource_filename("determined.deploy.gcp", "terraform/variables.tf")
    with open(fn, "r") as fin:
        # Poor man's HCL parser.
        m = re.search(r"master_config_template.+?EOF(.+?)EOF", fin.read(), re.DOTALL)
        if m:
            print(m.group(1).strip())
        else:
            raise RuntimeError("Failed to extract default master config template")
